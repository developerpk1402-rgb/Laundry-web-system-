...
const LoginPage: React.FC<{ onLogin: (user: User) => void }> = ({ onLogin }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    logAction(
      ...
      AuditAction.LOGIN,
      `User ${username} established secure session connection.`
    );
    ...
    <div className="min-h-screen bg-slate-900 dark:bg-black flex items-center justify-center p-4 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-slate-800 via-slate-900 to-black transition-colors duration-500">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-500 border border-white/5">
        <div className="bg-blue-600 dark:bg-blue-700 p-12 text-center text-white relative transition-colors duration-500">
          ...
          <h1 className="text-3xl font-black tracking-tight mb-2 italic tracking-tighter">LavanFlow OS</h1>
          <p className="text-blue-100 font-medium opacity-80 uppercase text-[10px] tracking-[0.4em]">Node Authentication</p>
        ...
        <form onSubmit={handleLogin} className="p-10 pt-16 space-y-6">
          <div className="space-y-4">
            <div>
              <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 ml-1">Username Handle</label>
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 ml-1">Secure Hash</label>
              <div className="relative group">
